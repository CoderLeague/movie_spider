...
import cmd
from toolkit.logkit import get_logger
import logging
import commands

logger = get_logger(logger_name = 'test', log_level=logging.DEBUG, print_level=logging.DEBUG)
...
class MediaInfo :
    ...
    def _ffmpegGetInfo(self) :
        cmd         = self.cmd +\
                    ' -loglevel quiet -print_format json -show_format -show_streams -show_error -count_frames -i ' +\
                    self.filename
        outputBytes = ''
...
#       except json.JSONDecodeError as err :
    ...
    def get_duration(self):
        ...
        获取失败, 返回-1
        """
        base_cmd = 'ffprobe "{filename}" -show_entries format=duration -v quiet -of csv="p=0" '
        cmd      = base_cmd.format(filename = self.filename)
        duration = commands.getoutput(cmd)
